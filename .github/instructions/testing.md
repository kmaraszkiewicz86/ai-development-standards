...
- [ ] xUnit, FluentAssertions, AutoFixture, NSubstitute installed
- [ ] `BaseFixture` created and shared
- [ ] Every test class has its own Fixture inheriting `BaseFixture`
- [ ] Tests follow Arrange / Act / Assert
- [ ] Tests named using `MethodName_Condition_ExpectedResult` pattern
- [ ] Happy path, validation, and exception cases covered
- [ ] Only external dependencies are mocked
- [ ] No shared mutable state between tests
- [ ] FluentAssertions used — not `Assert.Equal`
- [ ] No `.Result` or `.Wait()` in test code
